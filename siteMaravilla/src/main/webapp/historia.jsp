<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <title>Laguna de Yambo - Historia</title>
    <link rel="stylesheet" href="estilos/estilos.css">
</head>
<body>
    <header>
        <h1>Historia de la Laguna de Yambo</h1>
        <nav>
            <ul>
                <li><a href="index.jsp">Inicio</a></li>
                <li><a href="historia.jsp">Historia</a></li>
                <li><a href="galeria.jsp">Galería</a></li>
                <li><a href="evaluacion.jsp">Evaluación</a></li>
            </ul>
        </nav>
    </header>
    <main>
        <section>
            <h2>Historia</h2>
            <p>Se cree que la Laguna de Yambo fue formada por actividad volcánica antigua. Ha sido un sitio de importancia cultural y turística a lo largo de los años.</p>
        </section>
        <section>
            <h2>Atractivos</h2>
            <table border="1">
                <tr>
                    <th>Atractivo</th>
                    <th>Descripción</th>
                </tr>
                <tr>
                    <td>Tarabita</td>
                    <td>Una tarabita recorre parte de la laguna, 
                    	ofreciendo una vista aérea de la misma</td>
                </tr>
                <tr>
                    <td>Peseos en kayak o en barcos</td>
                    <td>Se pueden realizar recorridos en barcos con música
                    	y baile folklórico</td>
                </tr>
                <tr>
                    <td>Miradores</td>
                    <td>El mirador del Yambo ofrece vistas panorámicas de la laguna y los alrededores, 
                    	siendo un punto estratégico para tomar fotos y disfrutar del paisaje</td>
                </tr>
            </table>
        </section>
        <section>
            <h2>Lugares Cercanos</h2>
            <ol>
                <li>Salcedo</li>
                <li>Latacunga</li>
                <li>Ambato</li>
            </ol>
        </section>
    </main>
    <footer>
        <p>&copy; 2025 - Mario Alexander Salazar Serna</p>
    </footer>
</body>
</html>
